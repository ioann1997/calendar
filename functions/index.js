...
// Функция для генерации случайного ежедневного сообщения в 11:40
  ...
  async (event) => {
    ...
    try {
      ...
      for (const calendarDoc of calendarsSnapshot.docs) {
        ...
        // Ежедневное уведомление в 11:40 для всех пользователей PWA
        // Проверяем время более надежно: час = 11 и минута = 40
        const hourInt = parseInt(hour, 10);
        const minuteInt = parseInt(minute, 10);
        const is11AM = hourInt === 11 && minuteInt === 40;
        ...
        if (is11AM || currentTime === '11:40') {
          console.log(`✅ Время 11:40 обнаружено для календаря ${calendarId}, проверяем дату последней отправки...`);
          ...
          if (!last11AMDate || last11AMDate !== today) {
            if (fcmTokens.length === 0) {
              console.log(`⚠️ Нет токенов для отправки ежедневного уведомления 11:40`);
            } else {
              console.log(`🚀 Отправка ежедневного уведомления 11:40 для календаря ${calendarId}...`);
              ...
              try {
                ...
                const response = await admin.messaging().sendEach(messages);
                const result = processSendResponse(response, fcmTokens, calendarId, db);
                console.log(`✅ Ежедневное уведомление 11:40: отправлено ${result.successCount} уведомлений`);
                ...
                console.error('❌ Ошибка отправки ежедневного уведомления 11:40:', error);
                console.error('❌ Детали ошибки:', error.message, error.stack);
              }
            }
          } else {
            console.log(`⏭️ Ежедневное уведомление 11:40 уже отправлено сегодня для календаря ${calendarId} (${last11AMDate})`);
          }
        } else {
          // Логируем только если близко к 11:40 для отладки
          const hour = parseInt(currentTime.split(':')[0]);
          const minute = parseInt(currentTime.split(':')[1]);
          if (hour === 11 && minute >= 38 && minute <= 42) {
            console.log(`⏰ Время близко к 11:40, но не точно: ${currentTime}`);
          }
        }
      }
